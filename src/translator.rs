/// translator.rs — Rule-based AXON v2.0 translation engine
///
...
///   3. Detect negation
///   4. Detect causal / logical structure (→  ∴  ∵  ∧  ∨)
///   5. Tag each phrase token with its sigil  (@ # ~ $ ∅)
///   6. Assemble final AXON string + annotation
...
const NOISE_PHRASES: &[&str] = &[
    ...
    // ── Single-word politeness / filler (caught here as phrases for clarity) ─
    "please", "kindly",
];
...
impl Translator {
    pub fn new() -> Self {
        Self {
            ...
            code: CodeTranslator::new(),
        }
    }

    /// Main entry point
    pub fn translate(&self, input: &str) -> Translation {
        // ── 0. Try code-query translation first ──────────────────────
        ...
        // ── 1. Strip noise phrases (politeness, filler, meta-commentary) ─
    ...
    /// Remove noise phrases from text before the semantic pipeline runs.
    /// Handles start, end, and inline (comma-delimited) positions.
    /// Returns the cleaned string and the count of phrases removed.
    fn strip_noise(&self, text: &str) -> (String, usize) {
        ...
        while changed {
            ...
            for phrase in NOISE_PHRASES {
                ...
                // ── End of string ────────────────────────────────────────
                // Allow ", phrase" or ". phrase" suffix patterns
                for prefix in &[", ", " - ", ". ", "! ", "? "] {
                ...
                // ── Inline: ", phrase," or ", phrase" mid-sentence ───────
                for (open, close) in &[(", ", ", "), (", ", " "), (", ", "! "), (", ", "? ")] {
    ...
    fn parse_structure(&self, text: &str, negated: bool, annotations: &mut Vec<String>) -> String {
        ...
        if !universal.is_empty() {
            annotations.push("quantifier(∀)".into());
    ...
    fn extract_quantifier(&self, text: &str) -> (String, String) {
        ...
        for t in &universal_triggers {
            if text.starts_with(t) || text.contains(&format!(" {}", t.trim())) {
                ...
                return ("∀".into(), stripped.trim().to_string());
            }
        }
    ...
    fn tag_phrase(&self, phrase: &str, negated: bool) -> String {
        ...
        while i < words.len() {
            ...
            if i + 1 < words.len() {
                let bigram = format!(
                    "{} {}",
                    w_clean,
                    words[i + 1].to_lowercase().trim_matches(|c: char| !c.is_alphanumeric())
                );
                if self.concepts.contains(&bigram) {
                    let slug = bigram.replace(' ', "-");
        ...
        if tokens.is_empty() {
            // Fallback: take first meaningful word as concept
            for w in &words {
    ...
    fn classify_token(&self, lower: &str, original: &str, negated: bool) -> Option<String> {
        ...
        // (original starts with uppercase but it's not the first word context we handle here)
        ...
        // Known verb or normalised verb stem
