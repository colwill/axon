...
class ChatHistory {
  ...
  add(entry: Omit<ChatEntry, "id" | "timestamp">): ChatEntry {
  ...
  search(query: string): ChatEntry[] {
    const q = query.toLowerCase();
    return this.entries
      .filter(
        (e) =>
          e.userInput.toLowerCase().includes(q) ||
          e.axon.toLowerCase().includes(q) ||
...
// Manages a pseudoterminal that spawns `claude -p` processes, displays their
// output in a visible VSCode terminal, and streams responses back to callers.
...
class ClaudeTerminalBridge {
  ...
  ensureTerminal(): vscode.Terminal {
    ...
    const pty: vscode.Pseudoterminal = {
      ...
      handleInput: () => {
        // Read-only — input is driven by the extension
      },
    };
  ...
  async sendRequest(
    ...
    // Show a short summary in the terminal
    const displayText = text.length > 120 ? text.substring(0, 120) + "…" : text;
    this.writeEmitter.fire(`\x1b[36m❯\x1b[0m ${displayText}\r\n`);
    ...
    try {
      return await new Promise<string>((resolve) => {
        ...
        proc.stdout.on("data", (data: Buffer) => {
          ...
          fullOutput += chunk;
          // Echo to pseudoterminal (replace \n with \r\n for proper display)
        ...
        proc.on("error", (err) => {
          ...
          this.writeEmitter.fire(`\x1b[31m${msg}\x1b[0m\r\n`);
          this.busy = false;
          resolve(msg);
        });
      });
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const chat = vscode.chat.createChatParticipant(
    ...
    async (
      ...
      // Get a language model to respond
      ...
      if (!model) {
        // No VS Code LM available — fall back to Claude Code
      ...
      // Send the AXON to the model with the system prompt
      const messages = [
      ...
      try {
        ...
        if (err?.code === "NoPermissions") {
          ...
          // Fall back: send to Claude Code (tab first, then CLI)
          stream.markdown(
  ...
  // ── Open chat command (for the Ax button) ──────────────────────────────
  context.subscriptions.push(
  ...
  // ── Open chat in editor tab (WebviewPanel) ────────────────────────────
  ...
  // ── Direct translate commands (kept for power users) ───────────────────
  context.subscriptions.push(
    vscode.commands.registerCommand("axon.translate", async () => {
      ...
      if (!pick || pick.target === "preview") return;
...
/** Send text to the Claude Code VS Code extension window. */
async function sendToClaudeWindow(text: string): Promise<boolean> {
  try {
    // claude-vscode.editor.open accepts (sessionId, initialPrompt)
    // Pass undefined for sessionId to open a new conversation with the prompt
...
/** Send AXON prompt to claude CLI in the background and return its response. */
async function sendToClaudeTerminal(text: string): Promise<string> {
  const args = ["-p", "--output-format", "text"];
  ...
  return new Promise<string>((resolve) => {
    ...
    proc.stderr.on("data", (_data: Buffer) => {
      // stderr ignored for background fallback
    });
...
/** Try Claude Code window first, then CLI terminal fallback. */
...
class AxonSidebarProvider implements vscode.WebviewViewProvider {
  constructor(
